...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (isNaN(timestamp)) {
      return message.reply('Please provide a valid date/time string in the format "YYYY-MM-DD HH:mm".');
    }
    ...
    fs.writeFile(configPath, JSON.stringify(config, null, 2), (err) => {
      ...
      message.reply(`Next session time has been set! Use the !next command to see it. Here is the raw timestamp for you to verify: <t:${unixTimestamp}:F>`);
    });
  },
};
