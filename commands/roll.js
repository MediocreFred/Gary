...
module.exports = {
  ...
  execute(message, args) {
    console.log(
      ...
      `called the !roll command with the arguments ${args}`,
    );
    ...
    if (re.test(args) && args.length === 1) {
      ...
      if (numRolls !== "" && Number.parseInt(numRolls) <= 100) {
        ...
        message.channel.send("Too many rolls requested! (must be <= 100)");
      }
      ...
      if (rollResults.length !== 0) {
        ...
        message.channel.send(
          `${message.author.username}'s ${args} results: \`${prettyResults}\`\nTotal: \`${rollTotal}\``,
        );
      }
    } else {
