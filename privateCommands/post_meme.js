  ...
  execute(client, embed, subreddit) {
    ...
    request(
      ...
      (err, res, body) => {
        ...
        while (
          data.data.children[index].data.is_self !== false &&
          data.data.children[index].data.over_18 !== false &&
          index < 10
        ) {
          index = index + 1;
        }
        // get the post url
        const postUrl = `https://www.reddit.com/r/${subreddit}/comments/${data.data.children[index].data.id.replace(/['"]+/g, "")}`;
        // get the image url
        ...
        const postTitle = data.data.children[index].data.title.replace(/['"]+/g, "");

        // Save the meme
        fs.appendFileSync(
          path.resolve(__dirname, "../memeData", `${subreddit}.txt`),
          `\n${postImageUrl}`,
        );

        // Post the meme
        // Place known channel ID in list below
        const channelList = config.legacy_channels;
        for (const channel of channelList) {
          ...
          try {
            ...
            embed.setDescription(postTitle);
